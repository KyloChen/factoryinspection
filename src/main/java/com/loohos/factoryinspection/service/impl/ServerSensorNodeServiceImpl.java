...
public class ServerSensorNodeServiceImpl extends DaoSupport<ServerSensorNode> implements ServerSensorNodeService{
    ...
    @Override
    public List<Double> getLastSixNodeBySensorWithTypeAndCurDay(ServerSensor sensor, SensorType sensorType, int hours) {
        ...
        try{
            ...
            if (nodeList.size() > 0) {
                ...
                System.out.println("server无历史数据！");
                return null;
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    @Override
    public List<Date> getLastSixTimeBySensorAndCurDay(ServerSensor sensor, int hours) {
        Date curDate = new Date();
        long curDateS = curDate.getTime();
        long sixHoursAgoDateS = (curDate.getTime() - hours*60*60*1000);
        Date sixHoursAgoDate = new Date(sixHoursAgoDateS);
